    ...
    try {
      ...
      for (const q of questions) {
        ...
        formattedQuestions.push({
          ...
          correctAnswer: q.answerType === 'multiple_choice' ? String(q.correctAnswer ?? 0) : (q.openEndedAnswerType !== 'text' ? q.openEndedNumericAnswer?.trim() || '' : '')
        });
      }
                  ...
                  {question.answerType === 'multiple_choice' ? (
                    ...
                    <div className="space-y-4 rounded-2xl border border-blue-100 bg-blue-50/50 p-4">
                      <div className="flex flex-wrap gap-2">
                        <button
                          type="button"
                          onClick={() => updateQuestion(question.id, 'openEndedAnswerType', 'text')}
                        ...
                        <button
                          type="button"
                          onClick={() => updateQuestion(question.id, 'openEndedAnswerType', 'number')}
                          className={`rounded-full px-4 py-2 text-xs font-bold transition-all ${
                        ...
                        <button
                          type="button"
                          onClick={() => updateQuestion(question.id, 'openEndedAnswerType', 'selective')}
                          className={`rounded-full px-4 py-2 text-xs font-bold transition-all ${
